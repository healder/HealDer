...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: HealDer.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: healderd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizehealderamount=<n> " + _("Keep N HealDer anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. HealDer is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. HealDer is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("HealDer version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "HealDer server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of HealDer") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart HealDer to complete") << "\n";
    ...
    nAnonymizeHealDerAmount = GetArg("-anonymizehealderamount", 0);
    if(nAnonymizeHealDerAmount > 999999) nAnonymizeHealDerAmount = 999999;
    if(nAnonymizeHealDerAmount < 2) nAnonymizeHealDerAmount = 2;
    ...
    LogPrintf("Anonymize HealDer Amount %d\n", nAnonymizeHealDerAmount);
