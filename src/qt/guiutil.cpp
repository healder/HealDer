...
{
    // Convert HealDer:// to HealDer:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("HealDer://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "HealDer:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "HealDer.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for HealDer.lnk
...
{
    return GetAutostartDir() / "HealDer.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a HealDer.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=HealDer\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the HealDer app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add HealDer app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("HealDer-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  HealDer-qt [" + tr("command-line options") + "]                     " + "\n";
